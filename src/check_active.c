...
void bartlby_check_active(struct service * svc, char * cfgfile) {
	...
	*/
	...
	//Recieve Reply
	rmessage=malloc(sizeof(char)*(1024*2));
	...
	while((return_bytes=recv(client_socket, return_buffer, 1024, 0)) > 0 && connection_timed_out == 0) {
		return_buffer[return_bytes] = '\0';
		if(strlen(rmessage) + strlen(return_buffer) <= 1024*2) {
