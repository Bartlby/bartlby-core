	...
	if(shm_id != -1) {
		...
		if(strcmp(argv[2], "status") == 0) {
			...
			for(x=0; x<shm_hdr->svccount; x++) {
				printf("%ld;%s;%d;%s;%d;%s\n", svcmap[x].service_id, srvmap[svcmap[x].srv_place].server_name, srvmap[svcmap[x].srv_place].client_port, svcmap[x].service_name, svcmap[x].current_state,  svcmap[x].current_output);
			}
			exit(1);
